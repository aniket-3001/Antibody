...
async def _ensure_docs_ingested() -> None:
    """Self-heals Cloud Run's ephemeral disk: cognee dedupes by content (see
    help_api/ingest.py), so re-running this on every boot is a no-op once the
    graph already exists, but rebuilds it after a cold restart wipes local
    storage. Runs as a background task so it never blocks startup/health checks."""
    try:
        docs = [str(p) for p in Path("help_docs").glob("*.md")]
        if not docs:
            return
        await help_memory_service.add(docs)
        await help_memory_service.cognify()
        log.info("help docs graph ready (%d file(s))", len(docs))
    except Exception:
        log.warning("help docs auto-ingest failed", exc_info=True)
