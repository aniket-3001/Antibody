# Antibody ΓÇö what it is and how it works
...
- **Feed**: a live view of what's being reported right now, plus "emerging
  threats" ΓÇö families of scam that are picking up velocity.
...
- **Browser extension**: a popup + right-click "Check with Antibody" that
  calls the same read-only /scan endpoint ΓÇö scanning a page never adds it to
...
id" panel ΓÇö resetting hard-deletes your old id's reports and trust score from
the server and gives you a fresh one.

## Troubleshooting

- **"Unexpected token '<' is not valid JSON"**: this means the frontend tried
  to parse an HTML page as JSON ΓÇö almost always a dev-proxy misconfiguration
...
- **Leaderboard/My Reports show nothing**: these are scoped to your anonymous
  id. If you just reset your id, your history starts over ΓÇö this is expected,
  not a bug.
- **Verdict has no cited explanation**: the deterministic + semantic matching
  ...
  cited one ΓÇö the verdict itself is unaffected.
- **A report should be removed (false positive)**: `/report/{id}/forget`
  ...
  me" in the sidebar ΓÇö that's a hard delete of every report tied to that id.
